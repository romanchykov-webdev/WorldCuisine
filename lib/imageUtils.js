 ...
 * @returns {Promise<{uri: string, width: number, height: number, base64?: string}>} - Объект с результатом сжатия
 */
export async function compressImage100(
  uri,
  compress = 0.7,
  maxWidth = 1080,
  maxHeight = 1080,
 ...
 * @param _addImage
 * @param setAddImage
 * @param setLoadingCompresImg
 * @returns {{addImageRecipeList: ((function(): Promise<null|string|undefined>)|*)}}
 */
// export function useImagePicker(_addImage, setAddImage, setLoadingCompresImg) {
//   const addImageRecipeList = async () => {
//     try {
//       setLoadingCompresImg(true)
//
//       const res = await ImagePicker.launchImageLibraryAsync({
//         mediaTypes: ImagePicker.MediaTypeOptions.Images, // ✅ правильно
//         allowsEditing: true,
//         aspect: [1, 1],
//         quality: 1,
//       })
...
//       const originalUri = res.assets[0].uri
//
//       // Сжимаем (верни объект { uri }):
//       const compressedImage = await compressImage100(originalUri, 0.3)
//
//       // Хочешь размеры — замерь через FileSystem, либо оставь как есть
//       // const info = await FileSystem.getInfoAsync(compressedImage.uri)
//
//       // Обновляем состояние СТРОКОЙ
//       setAddImage(compressedImage.uri)
//
//       // Можно показать алерт про размеры, если посчитаешь их
//       // Alert.alert('Size image', `Compressed size: ${(info.size / (1024*1024)).toFixed(2)} MB`)
//
//       return compressedImage.uri // ✅ вернём строку
//     } catch (error) {
//       console.error('Error in image picking:', error)
//       Alert.alert('Error', 'Something went wrong while picking the image')
//       return null
...
// export function useImagePicker(addImage, setAddImage, setLoadingCompresImg, maxImages = 5) {
//   const addImageRecipeList = async () => {
//     // аккуратно проверяем массив
//     if (Array.isArray(addImage) && addImage.length >= maxImages) {
//       Alert.alert(i18n.t('You have reached the image limit for one item'))
//       return null
//     }
//
...
//
//       const originalUri = res.assets[0].uri
//       const compressedImage = await compressImage100(originalUri, 0.3) // -> { uri }
//
//       // Обновляем МАССИВ
//       setAddImage((prev) => {
//         const safe = Array.isArray(prev) ? prev : []
//         return [...safe, { uri: compressedImage.uri }]
//       })
//
...
// export function useImagePickerForRefactor(
//   currentImages,
//   setTempImages,
//   setLoading,
//   recipe,
